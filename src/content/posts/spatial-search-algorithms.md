...
![](/wp-content/uploads/2025/02/Three-nearest-neighbors-of-a-point.webp "某个点的三个最近邻点位")
...
![](/wp-content/uploads/2025/02/K-d-tree.webp "K-d 树的前三层")
...
![](/wp-content/uploads/2025/02/level-1.webp "第一层树节点")

接下来，我们“打开”离查询点最近的盒子，将其从队列中移除，并将它的所有子节点（较小的盒子）放回队列，与较大的盒子一起排序。

![](/wp-content/uploads/2025/02/level-1-2.webp "第一层，第二层树节点")

我们就这样继续，每次打开离查询点最近的盒子，并将它的子节点放回队列。当从队列中移除的最近项是一个实际的点时，它保证是最近的点。队列中第二个被移除的点将是第二近的点，依此类推。

![](/wp-content/uploads/2025/02/level-all.webp "所有层级的节点（包括点）混合在一起")
...
![](/wp-content/uploads/2025/02/lower-bound.webp "从一个点到一个盒子的距离是该盒子内所有点距离的下界")
...
![](/wp-content/uploads/2025/02/lower-bound-2.webp "从线段到边界框的距离作为盒子内点的距离下界")

我们只需要对算法进行一个修改，即将点对点和点对盒子的距离计算替换为线段对点和线段对盒子的距离计算。这种方法在2D 凹包计算库 **Concaveman** 中，也得到了运用。它接受一组点并生成如下所示的轮廓：

![](/wp-content/uploads/2025/02/Concave-hull.webp "Concaveman 生成的凹包（红色线）")
