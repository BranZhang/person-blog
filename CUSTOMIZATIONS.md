...
### 4. 图片资产 `public/`
- `src/assets/content-images/`：WordPress 迁移的正文图与封面图（684 个文件），由 Astro 图片管线生成响应式 `srcset`；`public/downloads/` 只保留 ZIP、MP3 等需要原路径下载的非图片资源。
...
### 8. 列表封面
- `src/content.config.ts`：posts schema 的 `cover` 使用 `image()`，正文、封面和 Gallery 均引用 `src/assets/content-images/` 下的本地图片并交由 `astro:assets` 处理。
...
- [x] **彻底清洗 WordPress 痕迹**：50 篇正文已全部从 Gutenberg 原始 HTML 转成干净的 Markdown/MDX（见第 11 项）。所有 `wp-block-*` / `wp-element-*` class、`<img>` 冗余属性、`<!--more-->` 分隔符已清除；图片已迁入 `src/assets/content-images/` 并改用相对资源路径，由 Astro 构建期优化。
